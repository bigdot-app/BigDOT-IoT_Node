...
IoT_Error_t iot_tls_connect(Network *pNetwork, TLSConnectParams *params)
{
  ...
  if((ret = network_connect(&tlsDataParams->server_fd.fd, pNetwork->tlsConnectParams.pDestinationURL, pNetwork->tlsConnectParams.DestinationPort)) != 0)
  {
    msg_error(" failed to connect to %s  ! socket_connect returned -0x%x\n", pNetwork->tlsConnectParams.pDestinationURL, -ret);
  ...
  mbedtls_ssl_set_bio(&(tlsDataParams->ssl), (void*) tlsDataParams->server_fd.fd, network_socket_send, network_socket_recv, NULL);
  msg_debug(" ok\n");
...
IoT_Error_t iot_tls_disconnect(Network *pNetwork)
{
  ...
  network_socket_close(pNetwork->tlsDataParams.server_fd.fd);
